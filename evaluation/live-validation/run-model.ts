...
import { verifyModelAssistedCompaction } from "./model-verify.js";
  ...
  try {
    ...
    try {
      ...
      if (first.observedEvents().some(event => event.type === "extension_error")) {
        throw new Error("Extension emitted extension_error during model-assisted compaction.");
      }
    } finally {
      await first.close();
    }

    const verification = await verifyModelAssistedCompaction(fixture.sessionFile, fixture.durableSourceId);
    ...
    const report = {
      ...
      modelAssistedOperations: ["observer", "reflector", "recall-agent-turn"],
