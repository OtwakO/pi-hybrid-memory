...
export const verifyModelAssistedCompaction = async (
  ...
  if (!details || details.version !== 5) throw new Error("Model-assisted compaction did not persist valid V5 details.");
  if (details.observationsRetired.length > 0) throw new Error("Model-assisted trial persisted an unexpected retirement.");
  if (details.reflectionsAdded.length === 0) throw new Error("Reflector persisted no new reflection.");
  ...
  if (reflectionIds.length === 0) throw new Error("No persisted reflection supports the held-out observation.");
