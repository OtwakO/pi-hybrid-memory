...
- **Controlled live validation is the current milestone.** The deterministic lifecycle gate now passes through Pi 0.84.3's real extension loader, two RPC compactions, persisted V5 journal, process restart, replay, exact provenance projection, idempotency, and Phase A fail-closed rollback inspection inside a repository-owned isolated session. It persisted exactly one exact-duplicate retirement, produced no unexpected observer records, and left the installed Phase A bundle and trial session unchanged during rollback inspection. A model-assisted observer/reflector/actual-tool-call trial is still required before actual-usage stability can be declared; its approved boundary is exactly three provider calls (catch-up observer, reflector, and post-restart `hm_recall` turn) against one exact held-out fixture. See `docs/CONTROLLED_LIVE_VALIDATION_ROADMAP_2026-08-27.md`. Effectiveness, disk, and cache refinement follow.
