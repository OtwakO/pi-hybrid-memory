...
- The current long session has exhausted effective observer-epoch headroom: the fresh baseline plus source was estimated at 108,898 tokens against the 108,800-token effective cap (`40% × 272,000`). Raising `observerEpochMaxTokens` further does not bypass the model-relative cap. The next immediate milestone is a localized capacity-aware source-segmentation correction that preserves the complete baseline, exact contiguous coverage, durable-write ordering, and fail-closed behavior. The verified design and token-efficient implementation/test sequence are tracked in `docs/OBSERVER_CAPACITY_ROADMAP_2026-08-27.md`.
- After that correction, proceed in isolated milestones: deterministic exact-duplicate retirement; preservation-safe reflection supersession; 300/600/900 quality harness; semantic retirement only if the harness justifies it; then compaction/disk/cache effectiveness tuning. See `docs/NEXT_SESSION_HANDOFF_2026-08-27.md` for the exact next-session sequence, ultimate memory/token/disk/cache objective, Pi-native-first architecture policy, maintainability standards, and tests.
