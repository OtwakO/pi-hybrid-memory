...
- **Controlled live validation is complete and the repository bundle is stable for actual usage through the controlled rollout procedure.** The deterministic gate passed Pi 0.84.3 extension loading, two RPC compactions, V5 persistence, restart replay, exact provenance, idempotency, and Phase A fail-closed rollback inspection. The model-assisted gate then passed catch-up observation, reflection persistence, restart replay, and an actual `hm_recall` tool invocation while preserving an exact held-out marker/path/value/constraint and source provenance. The installed Phase A bundle and real sessions remain untouched; upgrading them is a separate backed-up atomic rollout and fresh-process restart. Semantic retirement remains disabled. See `docs/CONTROLLED_LIVE_VALIDATION_ROADMAP_2026-08-27.md`. Real-session effectiveness, disk, and cache refinement follow.
