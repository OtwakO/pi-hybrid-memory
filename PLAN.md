...
- **Controlled live validation is the current milestone.** The approved first gate uses Pi 0.84.3's real extension loader, RPC compaction path, persisted SessionManager journal, process restart, replay, recall, and old-bundle fail-closed inspection inside a repository-owned isolated session directory. The installed Phase A bundle and all real sessions must remain byte-for-byte untouched. The deterministic first trial disables model folding and validates V5 exact-duplicate lifecycle persistence; a later model-assisted trial remains required before actual-usage stability can be declared. See `docs/CONTROLLED_LIVE_VALIDATION_ROADMAP_2026-08-27.md`. Effectiveness, disk, and cache refinement follow.
