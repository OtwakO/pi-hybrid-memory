...
- **Phase D semantic-retirement evaluation is complete and no protocol is approved.** The separate protocol retired all 12 deterministic required facts in its first 300-observation run. Combined runs were unstable: one produced zero false retirement with 84.61% reduction, while an identical repeat produced 9 required-fact failures/false retirements with only 0.70% reduction. The same model, fixture, and protocol therefore failed the safety and convergence gate. Semantic `fully-absorbed` and observation `superseded` retirement remain structurally disabled; 600/900 provider runs were intentionally skipped. See `docs/SEMANTIC_RETIREMENT_EXPERIMENT_ROADMAP_2026-08-27.md`. Effectiveness, disk, cache, and controlled live-install validation remain.
