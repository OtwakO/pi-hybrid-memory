...
- **Incremental reflection, V6 lifecycle persistence, local-only covered compaction, and the hard-bounded main-session projection are implemented, isolated-live validated, and installed for user testing.** Proactive durable observation progress triggers one bounded reflection transaction with request-local evidence handles and per-candidate provenance validation; one generalized branch-local V6 lifecycle sequence remains the sole persistence authority. Covered `/compact` makes zero extension model completions and performs only local replay, deterministic exact-duplicate retirement, VCC/projection assembly, fencing, and lifecycle persistence. The main projection always stays within `maxSummaryTokens`, discloses omitted durable memory through bounded recall IDs, and preserves parseable structural pressure state. Bundle `942c7bdb13a9f6a2ae6e41b2db497dba32916a447a3fb227b256b857ae4ebc95` passed 42 files / 312 tests, TypeScript, build, deterministic and model-assisted isolated Pi gates, and projected the real ~53,711-token long-session summary to 15,995/16,000 tokens with zero replay issues. It is installed at `~/.pi/agent/extensions/pi-hybrid-memory/index.js`; previous installed bundle `1f41a970…b01f` is backed up as `index.js.backup-20260829T054116Z`. Fresh-Pi actual-usage validation is now in progress. See `docs/INCREMENTAL_REFLECTION_AND_PROJECTION_ROADMAP_2026-08-28.md`.
