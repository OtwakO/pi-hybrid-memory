...
- **Real-session telemetry has exposed an observer-pipeline scaling defect and the bounded-pipeline redesign is the current milestone.** A ~95,059-token active baseline under a 96,000-token epoch left only hundreds of tokens for source, caused repeated cold capacity resets, and amplified one manual compaction into dozens of sequential catch-up completions; raising the cap to the model-relative 108,800 maximum increased source headroom but retained the unbounded baseline and routine confirmation completion. The approved direction is a bounded deterministic cache-stable observer context, one valid tool submission per normal segment, reuse of existing durable one-segment proactive progress, and at most one bounded observer segment per compaction attempt. Journal/lifecycle/recall schemas, semantic retirement, reflection redesign, embeddings, persistent indexes, generalized queues, and provider changes are out of scope. See `docs/BOUNDED_OBSERVER_PIPELINE_ROADMAP_2026-08-28.md`.
