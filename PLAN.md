...
- **G3 deep design is complete and awaiting approval.** The recommended target is a branch-local memory journal rather than repeated snapshots: observation evidence is written once in custom entries; successful V5 compactions add only new immutable reflection revisions and lifecycle events; the canonical branch index replays the active path into current/retired and current/superseded projections. See `docs/MEMORY_LIFECYCLE_DESIGN_2026-08-26.md`.
- The proposed implementation order is: V5 journal reader/writer with retirement disabled; deterministic exact-duplicate retirement; preservation-safe reflection supersession; then semantic retirement only after 300/600/900 quality fixtures. No persistence or retirement implementation starts until this design is approved.
