...
- **Incremental reflection and a hard-bounded main-session projection are the current structural milestone; Stage 1 bounded reflection is isolated-live validated but not installed.** Production evidence showed the prior reflector could consume ~88,892 input/~23,448 output tokens, propose 13 reflections, and persist zero after atomic invalid-provenance rejection; the same branch assembled a ~53,269-token summary despite a 16,000-token setting. Stage 1 keeps the current compaction persistence seam but bounds reflection to the pending compaction delta plus bounded historical context, assigns deterministic request-local evidence handles, permits at most four concise proposals with one structural correction, validates handles per candidate before the existing canonical merge, exits immediately on abort, and reports content-free plan/completion timing through `/hm-cache-info`. A read-only measurement on the current 1,351-observation branch selected all 136 pending focus observations into an estimated 24,667-token request with a 1,536-token output bound. The 36-file/283-test suite, typecheck, 45-module build, deterministic Pi gate, and model-assisted local-handle/reflection/restart/recall gate pass for repository bundle `3614d7ac21beee7c2407a71c488b6da67b747f362736d33f808179e194fd0f24`; the installed bundle remains `1f41a970…b01f`. Stage 3A–3B are implemented: one minimal V6 lifecycle shape can be stored in compaction details or a `hybrid-memory.lifecycle` custom entry; the sole branch projector enforces one parent-linked sequence, canonical observation-entry frontiers, same-policy monotonicity, atomic rejection, branch-local replay, and fail-closed legacy authority after V6 begins; successful compactions now emit V6 parented to the latest lifecycle entry while only carrying forward an existing frontier. The deterministic real Pi gate passed two V6 compactions, restart/replay, idempotent retirement, and byte-identical old-bundle inspection for repository bundle `10f6b387…cd3b`; the installed bundle remains unchanged and simply shows its older compatible projection. Stage 3C adds one synchronous `appendIncrementalLifecycle()` seam: it captures session/exact leaf/lifecycle head, replays the candidate through the sole projector, and appends one V6 custom entry only if all state is unchanged and valid; any intervening turn discards the result conservatively. It remains test-only and contains no inference or scheduling. Stage 3D adds pure `planNextReflectionWindow()`: it derives frontier state only from the branch projector, selects a contiguous canonical observation-entry prefix that fully fits the existing focus renderer, advances across empty/retired entries, restarts on policy-version change, and blocks rather than partially claiming an oversized entry. Stage 4A adds directly callable `processNextReflectionWindow()`: canonical replay → one bounded journal window → existing `foldMemory()` → strict V6 append. It advances empty windows without inference, persists only reflected/deliberate-empty/no-change results, defers below-threshold evidence, and leaves the journal unchanged on failures or stale state. Stage 4B wires proactive incremental reflection only after durable observer progress. It uses one reflection-specific task with at most one active run plus one latest coalesced rerun, never starts from compaction, never blocks `/compact`, and cancels/clears on session or branch lifecycle changes; strict processor fencing remains authoritative. Stage 4C exposes incremental reflection operational state without adding a new authority: `/hm-status` derives active/frontier/journal backlog/next-window state from the projector and planner; `/hm-cache-info` keeps one content-free normalized outcome with no branch or evidence IDs. Stage 4 is isolated-live validated for bundle `e5b61ce3…a4013`: proactive observation persisted exact evidence, incremental reflection `mtdg88w60001` persisted in V6 custom entry `32f81807` before compaction, compaction `6474e629` parented to it and added no reflection, and restart/exact recall preserved provenance. Installed bundle remains unchanged. Read-only long-session measurement confirms safe Stage 4 startup and continued projection pressure: the 65 MB branch has 718 unconsidered observation entries, 1,428 active observations (~103,855 tokens), zero reflections/V6 custom entries, no replay issues, and a ~53,711-token latest summary; its first bounded window fits 249 entries / 241 observations / ~14,748 focus tokens and is not blocked. Stage 5 is complete and isolated-live validated for bundle `7c959324…5a1f`: covered `/compact` is local-only and succeeds without an active model; it performs replay, deterministic duplicate retirement, VCC/projection assembly, fencing, and V6 persistence with zero model completions at the compaction interface. Gap catch-up resolves a model only when needed. Proactive reflection/restart/recall continuity also passes. Next: Stage 6 enforce a hard-bounded main-session projection; the real ~53,711-token summary under a 16,000-token setting means the overall redesign is not yet rollout-ready. Immutable observations, exact recall, no semantic retirement, one authoritative projector, no side database/index, and no generalized scheduler remain fixed constraints. See `docs/INCREMENTAL_REFLECTION_AND_PROJECTION_ROADMAP_2026-08-28.md`.
