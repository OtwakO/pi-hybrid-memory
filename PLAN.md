...
- **Incremental reflection and a hard-bounded main-session projection are the next structural milestone.** Production evidence shows the current reflector can consume ~88,892 input/~23,448 output tokens, propose 13 reflections, and persist zero after atomic invalid-provenance rejection; the same branch can assemble a ~53,269-token summary despite a 16,000-token setting. The approved staged direction first proves a bounded local-handle reflection contract at the existing `foldMemory()` seam, then adds the minimum generalized branch-local lifecycle event required for incremental persistence, removes reflector inference from `/compact`, and enforces a real projection ceiling. Immutable observations, exact recall, no semantic retirement, one authoritative projector, no side database/index, and no generalized scheduler remain fixed constraints. See `docs/INCREMENTAL_REFLECTION_AND_PROJECTION_ROADMAP_2026-08-28.md`.
