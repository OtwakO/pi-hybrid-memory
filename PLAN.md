...
- The current milestone is an opt-in Phase D experiment comparing combined and separate semantic-retirement protocols with `opencode-go/deepseek-v4-flash` through Pi's authenticated `ModelRuntime`. Dry-run is default, routine tests make no provider calls, reports are compact and gitignored, and no runtime retirement is enabled. See `docs/SEMANTIC_RETIREMENT_EXPERIMENT_ROADMAP_2026-08-27.md`. Disk/cache refinement remains later.
