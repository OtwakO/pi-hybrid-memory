...
After a successful run, the complete prior structured-message sequence is an exact prefix of the next request. This enables provider-independent prefix-cache reuse without removing prior memory, chronology, corrections, or provenance. The first observer call after upgrading to the Pi-native protocol intentionally cold-resets the disposable epoch so legacy agent-loop transcripts are never mixed with native completion transcripts; durable memory is unaffected.
...
Compaction catch-up uses an isolated fork: every uncovered chunk must succeed before one consolidated durable coverage entry is written. Deliberate-empty catch-up writes an empty entry so the branch still records that the full gap was examined. A failed catch-up discards the fork and cancels compaction; successful compaction invalidates the live epoch because reflection and branch boundaries may have changed.
