...
2. **G2 — Canonical branch memory index — Completed.** `buildBranchMemoryIndex()` now provides one read model for the latest committed-plus-pending current state, compactions, observation/reflection ID lookup, exact branch-entry lookup, observation provenance, and reflection-to-observation-to-source traversal. Compaction, observer, status, metrics, `/hm-memory`, and `hm_recall` use this seam; the old parallel readers were removed. Historical compaction parsing is lazy so ordinary turns do not pay recall-only costs. Known V3/V4 details remain supported and writes remain V4.
3. **G3 — Branch-local memory lifecycle design — Deep audit complete, awaiting approval.** The audit rejected repeated memory snapshots in favor of a V5 branch-local journal: observation evidence remains in one-time custom entries; successful compactions add only new immutable reflection revisions and lifecycle events; `buildBranchMemoryIndex()` replays the active branch into the current projection. Phase A writes V5 with retirement disabled, Phase B permits deterministic exact-duplicate retirement only, and semantic retirement remains gated by quality fixtures. See `docs/MEMORY_LIFECYCLE_DESIGN_2026-08-26.md`.
