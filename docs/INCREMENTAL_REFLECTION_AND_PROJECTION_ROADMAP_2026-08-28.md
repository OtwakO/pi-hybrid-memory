...
Next: remove reflection inference from compaction behind its own rollback boundary while preserving deterministic exact-duplicate retirement locally. Covered `/compact` still executes the compatibility reflector until Stage 5 lands, and the measured ~53,711-token summary confirms the hard projection ceiling remains mandatory after latency work.
