...
Next Stage 4 step: choose and implement the smallest trigger lifecycle. The default direction is one session-local reflection task coordinator invoked after successful durable observer progress, with no catch-up loop and no compaction wait. It should process at most one window per trigger, use session-local backoff for transient/unchanged failures, and expose backlog/outcome telemetry. Do not remove compaction reflection until the incremental path passes controlled restart/replay/model validation.
